...
function TableSelectMenu() {
  const toogleSelectMenu = useSelector((state:RootState) => state.toogleMenu.open)
  const toogleDarkMode= useSelector((state:RootState) => state.switchMode.darkMode)
  const fetchData = useSelector((state: RootState) => state.Boards);
  ...
  return (    
      <MenuWrapper visible={toogleSelectMenu} darkMode={toogleDarkMode} firstLoad={firstLoad}>
        ...
        <div className="boards-names-wrapper">
          <p className="boards-header">
              ALL BOARDS ({fetchData.boards.length})
          </p>
              <ul>
                {fetchData.boards.map(board => {
                  return(
                    <li key={board.name} className={activeBoard.name === board.name ? "board-name active": "board-name"} onClick={() => setActiveBoard(board)}>
                      <div className="board-icon">
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                          ...
                          fill={activeBoard.name === board.name ? "#FFFFFF" : "#828FA3"}/>
                        </svg>
                      </div>
              ...
              <ModeSwitch darkMode={toogleDarkMode} onClick={() => toogleDarkMode ? dispatch(toogleSwitch(false)) :  dispatch(toogleSwitch(true))}>
                <img className="mode-icon" src="./src/assets/icons/icon-light-theme.svg" alt="light" />
                <div className="switch">
                  <span className="circle"></span>
                </div>
                <img className="mode-icon" src="./src/assets/icons/icon-dark-theme.svg" alt="dark" />
              </ModeSwitch>
              <button className={toogleSelectMenu ? "btn-visible" : "btn-hidden" }onClick={() => toogleSelectMenu ? dispatch(toogle(false)) : dispatch(toogle(true))}>
                  <img className="btnIcon" src={`./src/assets/icons/${toogleSelectMenu ? "icon-hide-sidebar.svg" :"icon-show-sidebar.svg" }`} alt="" onClick={handleClick}/>
